...
namespace RedKite
{
    ...
    public class MeshMaker : MonoBehaviour
    {
        ...
        public void MakeMesh(Vector3 _scale, Vector3 _position)
        {
            position = _position;
            scale = _scale;
            ...
            subMeshes[0].uv = FindUVs(subMeshes[0], 0);
            subMeshes[1].uv = FindUVs(subMeshes[1], 1);
            subMeshes[2].uv = FindUVs(subMeshes[2], 2);
            subMeshes[3].uv = FindUVs(subMeshes[3], 3);
            subMeshes[4].uv = FindUVs(subMeshes[4], 4);
            subMeshes[5].uv = FindUVs(subMeshes[5], 5);

        }

        public void SetTextures(Renderer renderer, Texture2D[] textures)
        {
            /// <summary>
        ...
        Vector2[] FindUVs(Mesh mesh, int side)
        {
            Vector2[] uvs = new Vector2[mesh.vertexCount];
            int uvIndex = 0;

            float majorAxisLength = side == 4 | side == 5 ? (int)scale.z : (int)scale.x;

            float minorAxisLength = side == 1 | side == 3 ? (int)scale.z : (int)scale.y;


            for (int minor = 0; minor <= minorAxisLength; minor++)
            {
                for (int major = 0; major <= majorAxisLength; major++)
                {
                    if (uvIndex < mesh.vertexCount)
                    {
                        ...
                        Debug.Log(mesh.vertexCount + " " + uvIndex);
                }
            }
